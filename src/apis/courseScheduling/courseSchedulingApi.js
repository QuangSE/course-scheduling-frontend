...
class CourseSchedulingApi {
  ...
  getModuleByNameSemester(name, semester) {
    return this.api.post(MODULE + "/by-name-semester", {
      name: name,
      semester: semester,
    });
  }
  ...
  createModule(moduleName, semester, sws, visibility) {
    visibility = visibility ? visibility : semester % 2 == 0 ? 2 : 1;

    return this.api.post(MODULE, {
      name: moduleName,
      semester: semester,
      sws: sws,
  ...
  createDocent(title, firstName, lastName, email, job_type) {
  ...
  updateDocentCourse(id, docentCourseData) {
    return this.api.patch(DOCENT_COURSE + `/${id}`, docentCourseData);
  }
...
apiInstance = apiInstance ? apiInstance : new CourseSchedulingApi();
