  ...
  const importCsv = async function (csvData) {
    try {
      for (const line of csvData) {
        ...
        if (!moduleRes.data) {
          moduleRes = await api.createModule(
            line.modul,
            line.sem,
            line.modulsws
          );
        ...
        if (!erGroupIdRes.data) {
          erGroupIdRes = await api.createErGroup(
            line.studienganggruppe,
            examRegulationsId
          );
        ...
        let moduleErGroupRes = await api.getModuleErGroupByErGroupIdModuleId(
          erGroupId,
          moduleId
        );
        ...
        if (courseId && line.dozent) {
          ...
          if (docentRes.data) {
            ...
            docentCourseRes = await api.getDocentCourseByDocentIdCourseId(
              docentId,
              courseId
            );
            if (!docentCourseRes.data) {
              docentCourseRes = await api.createDocentCourse(
                docentId,
                courseId,
                0
              );
